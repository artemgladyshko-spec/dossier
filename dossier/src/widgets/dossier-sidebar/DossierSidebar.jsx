...
export function DossierSidebar({ groups }) {
  const { t } = useI18n()
  const [expandedSections, setExpandedSections] = useState(() => Object.fromEntries(groups.map((group) => [group.id, true])))
    ...
    >
      <Box sx={{ p: { xs: 2, md: 3 }, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        <List disablePadding sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {groups.map((group) => (
            <Box key={group.id} sx={{ overflow: 'hidden', bgcolor: 'background.paper' }}>
              <ListItemButton
              ...
              >
                <ListItemText
                  primary={<Typography variant="body2" sx={{ fontWeight: 700, color: 'text.primary' }}>{t(group.titleKey)}</Typography>}
                />
                  ...
                  {group.items.map((item) => (
                    <ListItemButton
                      ...
                      sx={{
                        ...
                        '&.Mui-selected:hover': { bgcolor: designTokens.colors.sidebarActive },
                      }}
                    >
                      <ListItemText
                        primary={<Typography variant="body2" color="text.secondary">{t(item.labelKey)}</Typography>}
                      />
                    </ListItemButton>
                  ))}
                </List>
